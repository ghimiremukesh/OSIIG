...
def initialize_hji_8D(dataset, minWith):
    def hji_8D(model_output, gt):
        ...
        lam14 = dvdx[:, 3:4] / ((12 + 12) / 2)  # lambda_14
        ...
        lam23 = dvdx[:, 6:7] / ((6 + 6) / 2)  # lambda_23
        ...
        max_acc_ux = torch.tensor([6.], dtype=torch.float32).to(device)
        min_acc_ux = torch.tensor([-6.], dtype=torch.float32).to(device)
        max_acc_uy = torch.tensor([12.], dtype=torch.float32).to(device)
        min_acc_uy = torch.tensor([-12.], dtype=torch.float32).to(device)
        max_acc_dx = torch.tensor([6.], dtype=torch.float32).to(device)
        min_acc_dx = torch.tensor([-6.], dtype=torch.float32).to(device)
        max_acc_dy = torch.tensor([4.], dtype=torch.float32).to(device)
        min_acc_dy = torch.tensor([-4.], dtype=torch.float32).to(device)
        RA = 1e-4
        RD = 1e-1
        ...
        ux[torch.where(ux < 0)] = -1
        ux[torch.where(ux == 1)] = max_acc_ux
        ux[torch.where(ux == -1)] = min_acc_ux
        # ux = -0.5 * lam13 / RA
        ...
        uy[torch.where(uy < 0)] = -1
        uy[torch.where(uy == 1)] = max_acc_uy
        uy[torch.where(uy == -1)] = min_acc_uy
        # uy = -0.5 * lam14 / RA
        ...
        dx[torch.where(dx < 0)] = -1
        dx[torch.where(dx == 1)] = min_acc_dx
        dx[torch.where(dx == -1)] = max_acc_dx
        # dx = 0.5 * lam23 / RD
        ...
        dy[torch.where(dy < 0)] = -1
        dy[torch.where(dy == 1)] = min_acc_dy
        dy[torch.where(dy == -1)] = max_acc_dy
        # dy = 0.5 * lam24 / RD
        ...
        vy1 = (model_output['model_in'][:, :, 4:5] + 1) * (12 + 12) / 2 - 12
        ...
        vx2 = (model_output['model_in'][:, :, 7:8] + 1) * (6 + 6) / 2 - 6
