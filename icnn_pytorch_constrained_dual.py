...
class FCBlock(nn.Module):
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features,
        ...
        # non-convex layer
        u_sizes = zip([in_features - 2] * 1 + [hidden_features] * (num_hidden_layers), [hidden_features] *
                      num_hidden_layers)
        ...
        yu_u_sizes = [in_features - 2] * 1 + [hidden_features] * (num_hidden_layers)
        self.net_yu_u = nn.ModuleList([nn.Sequential(nn.Linear(in_features, 2, bias=True), nn.Dropout(dropout)) for
                                       in_features in yu_u_sizes])

        z_yu_sizes = [hidden_features] * (num_hidden_layers) + [1]
        self.net_z_yu = nn.ModuleList([nn.Sequential(nn.Linear(2, out_features, bias=False), nn.Dropout(dropout))
                                       for out_features in z_yu_sizes])

        z_u_sizes = zip([in_features - 2] * 1 + [hidden_features] * (num_hidden_layers), [hidden_features] *
                        (num_hidden_layers) + [1])
    ...
    def forward(self, coords, params=None, **kwargs):
        ...
        u_input = coords[..., :-2]
        # check constraint
        violation = utils.check_violation(u_input[..., :2].detach().cpu().numpy(),
                                          u_input[..., 4:6].detach().cpu().numpy()).reshape(1, -1)
        # violation = np.expand_dims(violation, axis=-1)
        ...
        # return z
        output[violation] = -utils.PENALTY  # force to set the output to large number for violated states
        return output
